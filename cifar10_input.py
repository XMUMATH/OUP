...
IMG_DEPTH = 3
NUM_CLASS = 10
CUT_PERCENT = 0.9 # The cut percent for 10th class 
...
EPOCH_SIZE = 10000 * NUM_TRAIN_BATCH - int(CUT_PERCENT*(50000/10))
...
def read_in_all_images(address_list, shuffle = True, is_random_label = False, is_unbalance = True):
    ...
    num_data = len(label)
    cut_num = int(CUT_PERCENT*(num_data/10))
    ...
    if is_unbalance is True:
        ...
        # mark the examples which will be deleted
        index = 0
        del_list = []
        for i in label:
            if i == 9:
