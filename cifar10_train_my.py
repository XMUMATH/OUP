...
class Train(object):
    ...
    def placeholders(self):
        ...
        self.is_use_ckpt = tf.placeholder(dtype=tf.bool, shape=[])

        self.train_step = tf.placeholder(dtype=tf.int32, shape=[])
    ...
    def build_train_validation_graph(self):
        ...
        logits = inference(self.image_placeholder, FLAGS.num_residual_blocks, reuse = False, is_use_ckpt = self.is_use_ckpt, train_step = self.train_step)
        vali_logits = inference(self.vali_image_placeholder, FLAGS.num_residual_blocks, reuse = True, is_use_ckpt = self.is_use_ckpt, train_step = self.train_step)

        # The following codes calculate the train loss, which is consist of the
        # softmax cross entropy and the relularization loss
        regu_losses = tf.add_n(tf.get_collection('losses'), name = 'total_loss')
        loss = self.loss(logits, self.label_placeholder)
        self.full_loss = tf.add(loss, regu_losses)
        #train phase
        predictions = tf.nn.softmax(logits)
        self.train_top1_error = self.top_k_error(predictions, self.label_placeholder, 1, [], is_10th_top_1 = False)
        #validation phase
        self.vali_loss = self.loss(vali_logits, self.vali_label_placeholder)
        vali_predictions = tf.nn.softmax(vali_logits)
        label = self.vali_label_placeholder
        _, idx = tf.setdiff1d(label, [0,1,2,3,4,5,6,7,8]) 
        self.vali_top1_error, self.vali_10th_error_value = self.top_k_error(vali_predictions, self.vali_label_placeholder, 1, idx, is_10th_top_1 = True)
        #get fc variancen form collection
        w_std = tf.sqrt(tf.get_collection('fc_var'))
        self.fc_std = [tf.reduce_mean(w_std[0][0:8]),w_std[0][9]]
        ...
        for step in xrange(FLAGS.train_steps):
            ...
            if step % FLAGS.report_freq == 0:
                ...
                if FLAGS.is_full_validation is True:
                    ...
                    _, validation_error_value, validation_loss_value, fc_std, vali_10th_error_value = sess.run([self.val_op,
                                                                     self.vali_top1_error,
                                                                 self.vali_loss, self.fc_std,  self.vali_10th_error_value],
                                                {self.image_placeholder: train_batch_data,
                                                 ...
                                                 self.is_use_ckpt: FLAGS.is_use_ckpt,
                                                 self.train_step: step})

                val_error_list.append(validation_error_value)
                val_10th_error_list.append(vali_10th_error_value)
                fc_std_fir9_list.append(fc_std[0])
                fc_std_10th_list.append(fc_std[1])
            ...
            if step % FLAGS.report_freq == 0:
                summary_str = sess.run(summary_op, {self.image_placeholder: train_batch_data,
                                                    ...
                                                    self.lr_placeholder: FLAGS.init_lr,
                                                    self.is_use_ckpt: FLAGS.is_use_ckpt,
                                                    self.train_step: step})#self.is_use_ckpt: FLAGS.is_use_ckpt
                ...
                print 'Validation 10th top1 error = %.4f' % vali_10th_error_value
            ...
            if step == FLAGS.fine_tune_point:
                FLAGS.is_use_fine_tune == True
            ...
            if step % 10000 == 0 or (step + 1) == FLAGS.train_steps:
                ...
                df = pd.DataFrame(data={'step':step_list, 'train_error':train_error_list,
                                'vali_error': val_error_list, 'vali_10th_error': val_10th_error_list, 'fc_std_fir9':fc_std_fir9_list, 'fc_std_10th':fc_std_10th_list})
    ...
    def top_k_error(self, predictions, labels, k, idx, is_10th_top_1 = True):
        '''
        ...
        '''
        if is_10th_top_1 is True:
            batch_size = predictions.get_shape().as_list()[0]
            _10th_batch_size = 500
            in_top1 = tf.to_float(tf.nn.in_top_k(predictions, labels, k=1))
            num_correct = tf.reduce_sum(in_top1)
            _10th_num_correct = tf.reduce_sum(tf.gather(in_top1, idx))
            return (batch_size - num_correct) / float(batch_size), (_10th_batch_size - _10th_num_correct) / float(_10th_batch_size)
        else:
            batch_size = predictions.get_shape().as_list()[0]
            in_top1 = tf.to_float(tf.nn.in_top_k(predictions, labels, k=1))
            num_correct = tf.reduce_sum(in_top1)
    ...
    def generate_vali_batch(self, vali_data, vali_label, vali_batch_size):
        ...
        '''
        offset = np.random.choice(10000 - int(CUT_PERCENT*(10000/10) ) - vali_batch_size, 1)[0]
